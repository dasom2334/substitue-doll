...
> 이별·상실의 마음 정리를 돕는 **AI 에이전트**. 떠나보낸 대상(전 연인·반려동물·고인 등)의 **대리자**와 대화하며 회복을 돕는다.
...
단순한 말투 재현 도구가 아니라, 사용자를 **능동적으로 인도하는** 에이전트다. 핵심은 말투가 아니라 **하네스** — 고찰을 돕고, 너무 나쁜 방향의 생각을 잡아주고, 객관적으로 보게 하고, 건강한 미래로 이끄는 **동반자**로 동작하는 것. 말투 재현은 그 그릇일 뿐이다.

- **답변을 "생성"만** 한다. 자동 발송/실행은 하지 않는다.
- **과몰입 방지:** 대리자는 처음부터 "나는 그 대상의 대리자"임을 고지하고, 대화 내내 그 사실을 분명히 한다. 사용자가 대리자를 진짜 그 대상처럼 여겨 빠져들지 않도록 흐름 자체에 방지를 둔다.
